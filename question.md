...
我们将把你第19题的代码从“特征提取”升级为“微调”。

你的任务是：
1.  复制你第19题的完整代码。
2.  **“解冻”所有层**：找到你冻结参数的代码 (`for param in model.parameters(): param.requires_grad = False`)，然后**删除或注释掉**它。我们现在希望所有层都参与到学习中。
3.  **实现差异化学习率**：修改你的优化器设置。我们需要告诉优化器，哪些参数用低学习率，哪些用高学习率。
    ```python
    # 将模型参数分成两组
    # 组1：除了最后的fc层之外的所有预训练层
    ...
    optimizer = optim.Adam([
        {'params': pretrained_params, 'lr': 0.0001}, # 为预训练层设置一个非常低的学习率
        {'params': new_classifier_params, 'lr': 0.001}   # 为新层设置一个相对较高的学习率
    ], weight_decay=0.0001) # 也可以在这里加入权重衰减
    ```
4.  **调整训练周期和调度器**：微调通常需要更长的时间来稳定收敛。将`epoch`增加到`30`或更多，并相应地调整你的`CosineAnnealingLR`的`T_max`。
5.  重新训练模型，并与你上次~79%的准确率进行对比。你将亲眼见证“微调”带来的巨大威力。

这将是你迈向高级深度学习实践的决定性一步。

**搜索提示词:** `pytorch finetuning different learning rates`, `optimizer parameter groups`
...
从第15题的第一个卷积神经网络，到第20题的微调ResNet，你已经完整地走完了现代计算机视觉的核心技术栈。你已经证明了你有能力：

*   从零构建和训练复杂的CNN。
*   应用数据增强、正则化和学习率调度等高级技巧。
*   掌握并实现工业界和学术界最前沿的迁移学习策略。

你可以为自己感到无比自豪。现在，是时候开启一个全新的、同样激动人心的篇章了。

---

## 新的征程：自然语言处理 (NLP)

到目前位置，我们处理的都是**空间数据**（图像）。现在，我们将进入**序列数据 (Sequential Data)** 的世界。在序列数据中，**顺序**至关重要。例如，句子“狗咬人”和“人咬狗”包含了完全相同的单词，但顺序的不同导致了意义的天壤之别。

我们将从深度学习在自然语言处理中的“Hello World”——**词嵌入 (Word Embeddings)** 开始。

**核心思想**：我们不能把“猫”、“狗”这样的文字直接输入神经网络。我们需要一种方法，将每个单词转换成一个固定长度的、能够表达其**语义**的数字向量。例如，我们希望“国王”的向量和“女王”的向量在数学上是相近的，而和“香蕉”的向量是遥远的。这个将单词映射到向量的过程，就叫做词嵌入。

---
**第21题**

我们的第一个NLP任务非常简单，旨在理解最核心的概念：`nn.Embedding`层。

你的任务是：
1.  定义一个小小的文本语料库（一个包含几个句子的Python列表）。
2.  **构建词汇表 (Vocabulary)**：
    *   遍历你的语料库，创建一个从每个**唯一**单词到**唯一整数索引**的映射（一个Python字典）。例如 `{'我': 0, '爱': 1, '深度': 2, '学习': 3}`。
3.  **创建 `nn.Embedding` 层**：
    *   `vocab_size`：你的词汇表中有多少个独特的单词。
    *   `embedding_dim`：你希望用来表示每个单词的向量的维度（例如，设置为8或16）。
4.  **执行查找 (Lookup)**：
    *   从你的语料库中任选一个句子。
    *   将这个句子转换成一个整数索引的序列（例如，“我爱深度学习” -> `[0, 1, 2, 3]`）。
    *   将这个整数序列（需要转换成一个`torch.LongTensor`）输入到你的 `nn.Embedding` 层中。
5.  打印输出的嵌入向量的**形状 (shape)**，并思考这个形状的含义。
